...
error_rate_fixed_select <- function(num_loops,N,phi,theta,trt_diff,select_diff,
  ...
  for (n_loop in 1:num_loops){
    ...
    dataset_cum <- list(df)
    test_stats <- test_select_diff(dataset_cum,1)
...
stopping_sample_size_select <- function(num_loops,phi,theta,trt_diff,select_diff,
  ...
  type1_error <- error_rate_select(N_error_rate,x$upper$bound,
                                   x$lower$bound,N,phi,theta,trt_diff,0,
                                   prefer_diff,mu,q,K,q_cum)
  
  # GSD empirical power
  GSD_power <- error_rate_select(N_error_rate,x$upper$bound,
  ...
  if (sfu_type == "OF"){
    ...
    fixed_type_1 <- error_rate_fixed_select(N_error_rate,n,phi,theta,trt_diff,0,
                                           prefer_diff,mu,alpha_overall)
    
    # Fixed design empirical power
    fixed_power <- error_rate_fixed_select(N_error_rate,n,phi,theta,trt_diff,select_diff,
                                          prefer_diff,mu,alpha_overall)
  }
  else{
    fixed_type_1 <- 0
...
for (i in 1:length(Selection_Difference)){
  ...
  # Pocock result 
  ret_pocock <- stopping_sample_size_select(N_stopping,phi,theta,trt_diff,
                                            Selection_Difference[i],
                                            prefer_diff,mu,K,"Pocock",timing_info,N_error_rate)
  ...
  # OBF result 
  ret_OBF <- stopping_sample_size_select(N_stopping,phi,theta,trt_diff,
                                         Selection_Difference[i],
                                         prefer_diff,mu,K,"OF",timing_info,N_error_rate)
