...
# list of first analysis information fraction
timing_info_lst <- seq(0.3,0.8,0.02)
...
cum_score_treat <- function(dataset_cum, theta,q_cum,K,N){
  ...
  for (i in 1:K){
    ...
    tau_val <- (mean(df$Y[df$Treatment == 1 & df$first_stage == "Random"]) -
      ...
      sqrt(var(df$Y[df$Treatment == 1 & df$first_stage == "Random"])/n_1 +
             var(df$Y[df$Treatment == 0 & df$first_stage == "Random"])/n_2)
...
stopping_sample_size <- function(num_loops,phi,theta,trt_diff,select_diff,
  ...
  return(list(ret_stopping_size,n,type1_error))
}
...
for (i in 1:length(timing_info_lst)){
  
  # Pocock result
  ret_pocock <- stopping_sample_size(N_trial,phi,theta,
                                     trt_diff,
                                     select_diff,prefer_diff,mu,K,"Pocock",c(timing_info_lst[i]))
  Sequential_Sample_Size_Pocock[i] <- ceiling(mean(ret_pocock[[1]]))
  Fixed_Sample_Size[i] <- ceiling(ret_pocock[[2]])
  
  # OBF result
  ret_OBF <- stopping_sample_size(N_trial,phi,theta,
                                  trt_diff,
                                  select_diff,prefer_diff,mu,K,"OF",c(timing_info_lst[i]))
  Sequential_Sample_Size_OF[i] <- ceiling(mean(ret_OBF[[1]]))
}

df <- cbind(timing_info_lst,Sequential_Sample_Size_Pocock,Sequential_Sample_Size_OF,
            Fixed_Sample_Size)
df <- data.frame(df)
colnames(df) <- c('Timing', 'Pocock Average Sample Size', 'OBF Average Sample Size','Fixed Sample Size')
View(df)

values <- c(Sequential_Sample_Size_Pocock,Sequential_Sample_Size_OF,Fixed_Sample_Size)
group <- c(rep("Pocock Average Sample Size",length(timing_info_lst)),
           rep("OBF Average Sample Size",length(timing_info_lst)),rep("Fixed Design Sample Size",
                                                              length(timing_info_lst)))
results <- data.frame(timing_info_lst,group,values)

# plot
g_treat <- ggplot(data = results, aes(x=timing_info_lst, y=values, group = group)) +geom_line(aes(color=group))+
  geom_point(aes(color=group))  + labs(title =
  "Test for Treatment Effect", x = "First Analysis Information Fraction",
  y = "Sample Size") + theme(plot.title = element_text(size=12))
...
# Calculate empirical Type I error rate and empirical power
...
stopping_sample_size_select <- function(num_loops,phi,theta,trt_diff,select_diff,
  ...
  return(list(ret_stopping_size,ceiling(n),type1_error))
}
...
for (i in 1:length(timing_info_lst)){
  ...
  ret_pocock <- stopping_sample_size_select(N_trial,phi,theta,trt_diff,
                                            select_diff,
                                            prefer_diff,mu,K,"Pocock",c(timing_info_lst[i]))
  
  Seq_Size_Pocock_select[i] <- ceiling(mean(ret_pocock[[1]]))
  Fixed_Size_select[i] <- ret_pocock[[2]]
 
  # OBF Stopping Sample Size 
  ret_OBF <- stopping_sample_size_select(N_trial,phi,theta,trt_diff,
                                         select_diff,
                                         prefer_diff,mu,K,"OF",c(timing_info_lst[i]))
  Seq_Size_OF_select[i] <- ceiling(mean(ret_OBF[[1]]))
}

df_select <- cbind(timing_info_lst,Seq_Size_Pocock_select,Seq_Size_OF_select,Fixed_Size_select)
df_select <- data.frame(df_select)
colnames(df_select) <- c('First Analysis (Timing)', 'Pocock Average Sample Size', 'OBF Average Sample Size', 'Fixed Sample Size')
View(df_select)

values <- c(Seq_Size_Pocock_select,Seq_Size_OF_select,Fixed_Size_select)
group <- c(rep("Pocock Average Sample Size",length(timing_info_lst)),
           rep("OBF Average Sample Size",length(timing_info_lst)),
           rep("Fixed Design Sample Size",length(timing_info_lst)))
results <- data.frame(timing_info_lst,group,values)

# plot
g_select <- ggplot(data = results, aes(x=timing_info_lst, y=values, group = group)) +geom_line(aes(color=group))+
  geom_point(aes(color=group))  + labs(title = "Test for Selection Effect",
                                       x = "First Analysis Information Fraction",
                                       y = "Sample Size") + theme(plot.title = element_text(size=12))
...
for (i in 1:length(timing_info_lst)){
  # Pocock Stopping Sample Size 
  ret_pocock <- stopping_sample_size_prefer(N_trial,phi,theta,trt_diff,
                                            select_diff,
                                            prefer_diff,mu,K,"Pocock",c(timing_info_lst[i]))
  ...
  # OBF Stopping Sample Size 
  ret_OBF <- stopping_sample_size_prefer(N_trial,phi,theta,trt_diff,
                                         select_diff,
                                         prefer_diff,mu,K,"OF",c(timing_info_lst[i]))
  Seq_Size_OF_prefer[i] <- ceiling(mean(ret_OBF[[1]]))
}

df_prefer <- cbind(timing_info_lst,Seq_Size_Pocock_prefer,Seq_Size_OF_prefer,Fixed_Size_prefer)
df_prefer <- data.frame(df_prefer)
colnames(df_prefer) <- c('First Analysis (Timing)', 'Pocock Average Sample Size', 'OBF Average Sample Size',
                         'Fixed Sample Size')
View(df_prefer)

values <- c(Seq_Size_Pocock_prefer,Seq_Size_OF_prefer,Fixed_Size_prefer)
group <- c(rep("Pocock Average Sample Size",length(timing_info_lst)),
           rep("OBF Average Sample Size",length(timing_info_lst)),
           rep("Fixed Design Sample Size",length(timing_info_lst)))
results <- data.frame(timing_info_lst,group,values)
...
# plot
g_prefer <- ggplot(data = results, aes(x=timing_info_lst, y=values, group = group)) +geom_line(aes(color=group))+
  geom_point(aes(color=group))  + labs(title = "Test for Preference Effect",
                                       x = "First Analysis Information Fraction",
                                       y = "Sample Size") + theme(plot.title = element_text(size=12))

# combine plots
g_combine <- ggarrange(g_treat,g_select,g_prefer,ncol=1,heights=2)
annotate_figure(g_combine, top = text_grob("Change of Sample Size by First Analysis Information Fraction", 
